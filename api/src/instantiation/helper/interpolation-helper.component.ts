...
export class InterpolationHelper {
    ...
    interpolateFile(fullPath: string, featerVariables: FeaterVariablesSet): string {
    ...
    protected interpolateFeaterVariables(text: string, featerVariables: FeaterVariablesSet): string {
        return _.reduce(
            featerVariables.toList(),
            (reducedText, {name, value}) => {
                const tokenRegExp = new RegExp(
                    escapeStringRegexp(`${this.START_DELIMITER}${name}${this.END_DELIMITER}`),
                    'g',
                );
