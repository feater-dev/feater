...
export class DeployKeyDetailComponent implements OnInit {
    ...
    regenerateDeployKey(): void {
        ...
        this.apollo
            .mutate({
                mutation: gql`
                    ${this.getRegenerateDeployKeyMutation()}
                `,
            })
            .subscribe(
                () => {
                    this.toastr.success(
                        `Deploy key for <em>${this.deployKey.cloneUrl}</em> regenerated.`,
                    );
                    this.spinner.hide();
                    this.getDeployKey();
                },
                () => {
                    this.toastr.error(
                        `Failed to regenerate deploy key for <em>${this.deployKey.cloneUrl}</em>.`,
                    );
                    this.spinner.hide();
                    this.getDeployKey();
                },
    ...
    removeDeployKey(): void {
        this.dialogService
            .addDialog(ConfirmComponent, {
                title: 'Confirm',
                message:
                    'Are you sure you wish to remove this deploy key? This operation cannot be reversed.',
                ok: 'Confirm removal',
                cancel: 'Cancel',
            })
            .subscribe(isConfirmed => {
                ...
                this.apollo
                    .mutate({
                        mutation: gql`
                            ${this.getRemoveDeployKeyMutation()}
                        `,
                    })
                    .subscribe(
                        () => {
                            this.spinner.hide();
                            this.toastr.success(
                                `Deploy key for <em>${this.deployKey.cloneUrl}</em> removed.`,
                            );
                            this.router.navigate(['/deploy-keys']);
                        },
                        error => {
                            this.toastr.error(
                                `Failed to remove deploy key for <em>${this.deployKey.cloneUrl}</em>.`,
                            );
                            this.getDeployKey();
                        },
                    );
    ...
    protected getDeployKey(): void {
        ...
        this.apollo
            ...
            .valueChanges.subscribe(result => {
                const resultData: GetDeployKeyDetailQueryInterface =
                    result.data;
