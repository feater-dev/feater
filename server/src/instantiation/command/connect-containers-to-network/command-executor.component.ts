...
@Injectable()
export class ConnectToNetworkCommandExecutorComponent
    ...
    async execute(command: SimpleCommand): Promise<any> {
        ...
        const dockerInspectStdout = execSync(`docker inspect ${containerId}`, {
            maxBuffer: BUFFER_SIZE,
        });

        const inspectResult = JSON.parse(dockerInspectStdout.toString())[0];
        const ipAddress =
            inspectResult.NetworkSettings.Networks[
                config.instantiation.proxyNetworkName
            ].IPAddress;
