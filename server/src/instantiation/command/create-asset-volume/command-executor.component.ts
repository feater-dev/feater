...
@Injectable()
export class CreateAssetVolumeCommandExecutorComponent
    ...
    async execute(command: SimpleCommand): Promise<unknown> {
        ...
        envVariables.add(
            `FEATER__ASSET_VOLUME__${assetVolumeId.toUpperCase()}`,
            assetDockerVolumeName,
        );
        ...
        featerVariables.add(
            `asset_volume__${assetVolumeId.toLowerCase()}`,
            assetDockerVolumeName,
        );
    ...
    private extractAssetToVolume(
        ...
        createReadStream(absoluteUploadedAssetGuestPath).pipe(
            spawnedExtractAndCreateVolume.stdin,
        );
