...
export class DefintionRecipeZeroOneZeroValidator {
    ...
    protected createSchema(): joi.Schema {
        return joi.object().keys({
            schema_version: joi.string().equal('0.1.0').required(),
            sources: joi.array().items(this.createSourceSchema()).required(),
            source_volumes: joi.array().items(this.createSourceVolumeSchema()).required(),
            asset_volumes: joi.array().items(this.createAssetVolumeSchema()).required(),
            proxied_ports: joi.array().items(this.createProxiedPortSchema()).required(),
            env_variables: joi.array().items(this.createEnvVariableSchema()).required(),
            compose_files: joi.array().items(joi.object()), // TODO
            after_build_tasks: joi.array().items(joi.object()), // TODO
            summary_items: joi.array().items(this.createSummaryItemSchema()).required(),
        });
    }
    ...
    protected createSourceSchema(): joi.Schema {
        return joi.object().keys({
            ...
            reference: joi.object().keys({
                type: joi.string().equal(referenceTypes).required(),
