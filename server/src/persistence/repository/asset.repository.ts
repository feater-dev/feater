...
export class AssetRepository {
    constructor(
        @InjectModel('Asset')
        private readonly assetModel: Model<AssetInterface>,
    ...
    async findOneOrFail(criteria: any): Promise<AssetInterface> {
    ...
    async findByProjectIdAndIdentifierOrFail(
        ...
        if (!asset) {
            throw new Error(
                `Asset document with project ID ${projectId} identifier ${identifier} not found.`,
            );
        }
    ...
    create(
        createAssetInputType: CreateAssetInputTypeInterface,
    ): Promise<AssetInterface> {
    ...
    async removeByProjectIdAndIdentifier(
        projectId: string,
        identifier: string,
    ): Promise<boolean> {
        const asset = await this.findByProjectIdAndIdentifierOrFail(
            projectId,
            identifier,
        );
